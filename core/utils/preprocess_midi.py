...
from core.utils.encoding import pit2str, dur2str, trk2str, ins2str, pit2alphabet,bom2str,pos2str
...
'''
The preprocess_midi.py multi-process all the Midis and convert them into a raw_corpus.txt file. 
In this file, each line of encoded text represents a full song.
'''
...
def get_init_note_events_io(pm, p_midi):  # extract all notes in midi file
            ...
            if note_dur / max_dur > 1:
                ...
                while total_dur != 0:
                    if total_dur > max_dur:
                        note_events.extend([[round(start*440), "ON", note.pitch, instrument.program,
                                             instrument.is_drum, track_idx, round(max_dur*440)]])

                        note_on_ticks.append(round(start*440))
                        note_dur_lst.append(round(max_dur*440))

                        start += max_dur
                        total_dur -= max_dur
                    else:
                        note_events.extend([[round(start*440), "ON", note.pitch, instrument.program,
                                             instrument.is_drum, track_idx, round(total_dur*440)]])
                        note_on_ticks.append(round(start*440))
                        note_dur_lst.append(round(total_dur*440))

                        total_dur = 0

            else:
                note_events.extend(
                    [[round(note.start*440), "ON", note.pitch, instrument.program, instrument.is_drum, track_idx, round(note_dur*440)]])

                # for score analysis and beat estimating when score has no time signature
                note_on_ticks.append(round(note.start*440))
                note_dur_lst.append(round(note.end - note.start)*440)
...
def calculate_measure(p_midi, first_event_tick,
    ...
    if not time_signature_changes:  # no time_signature_changes, estimate it
        ...
        for idx in range(len(time_signature_changes)):
            ...
            numerator = time_signature_changes[idx].numerator# 4
            denominator = time_signature_changes[idx].denominator# 4
            ticks_per_measure = p_midi.ticks_per_beat * (4 / denominator) * numerator # p_midi.ticks_per_beat均为220

            cur_tick = time_signature_changes[idx].time# 0

            if idx < len(time_signature_changes) - 1:
                next_tick = time_signature_changes[idx + 1].time
            else:
                next_tick = last_event_tick + int(ticks_per_measure)# last_event_tick+880
...
def midi_to_event_seq_str_io(pm, pm_total, midi_file_path, readonly=False, use_control=False):
    p_midi = MidiFile(midi_file_path)

    start=0;end=0
    note_events, note_on_ticks, _ = get_init_note_events_io(pm,p_midi)
    start_note=note_events[0]
    end_note=note_events[-1]

    note_events_total, note_on_ticks_total , _ = get_init_note_events_io(pm_total,p_midi)

    for idx,item in enumerate(note_events_total):
        if start_note==item:
            start=idx
        if end_note==item:
            end=idx
...
def midi_to_event_seq_str(midi_file_path,readonly=False, use_control=False):
    p_midi = MidiFile(midi_file_path)
